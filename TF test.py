...
true_y0 = torch.tensor([[2.0, 0]]).to(device)
...
class Lambda(nn.Module):
    def forward(self, t, y):
        ...
        # return torch.mm(y3, true_A)
        # return torch.mm(y ** 3 + t, true_A)#这里我们可以在表达式中加入新的t，至少不会在计算中报错，是不是也可以加入别的参数？
        # 原始公式里面的true_A,是不是就是我们可以设置我们需要的参数的地方，但是经过初步的尝试，发现以这个程序的架构，我们没有办法传入这个参数


with torch.no_grad():
    true_y = odeint(Lambda(), true_y0, t, method='dopri5')  # 关闭自动求解梯度，可以加快速度，这个变量应该自然而然就是在gpu上的
    """true_y.shape = 1000,1,2"""
