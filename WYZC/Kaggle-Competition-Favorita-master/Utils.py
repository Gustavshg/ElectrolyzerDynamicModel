...
def load_data():
    ...
    df_train = pd.read_csv('train.csv',dtype={'onpromotion': bool},parse_dates=["date"],engine = 'pyarrow')
    # df_train = pd.read_csv('train.csv', usecols=[1, 2, 3, 4, 5], dtype={'onpromotion': bool},
    #                        converters={'unit_sales': lambda u: np.log1p(float(u)) if float(u) > 0 else 0},
    #                        parse_dates=["date"],engine = 'python')
    df_test = pd.read_csv("test.csv",dtype={'onpromotion': bool},parse_dates=["date"],engine = 'pyarrow')
    # subset data
